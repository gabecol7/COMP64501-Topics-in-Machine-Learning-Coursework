...
def objective(trial):
    ...
    batch_size = 64 #To speed up training
    lr = trial.suggest_float("lr", 1e-4, 1e-1, log=True)
    kernel_size = trial.suggest_categorical('kernel_size', [3,5]) 
    depth = trial.suggest_int('depth', 0, 20, step=1)
    dim = trial.suggest_int('dim', 1, 64)
    coarse_depth = trial.suggest_int('coarse_depth', 0, 3)
    fc_size = trial.suggest_categorical("fc_size", [4,8,16,32,64])
    ...
    #To speed up optimisation return (num_params/100000)**2 if an infeasible model class is produced (num_params > 100000)
    # without running model providing a lower bound for the val loss
    if num_params > 100000:
        return (num_params/100000)**2
    else:
        ...
        return mean_val_loss + (num_params/100000)**2
...
def main():
    ...
    df.to_csv('Hyperparameter Optimisation Results.csv')
